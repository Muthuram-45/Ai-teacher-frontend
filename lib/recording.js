...
async function uploadChunks(chunks, roomName, className, partNum, isFinal = false) {
    ...
    if (isFinal) {
        // 📥 Instantly download the video to the user's local machine
        const videoUrl = URL.createObjectURL(blob);
        const a = document.createElement("a");
        a.style.display = "none";
        a.href = videoUrl;
        a.download = `Class_Recording_${className}_${new Date().toLocaleDateString().replace(/\//g, "-")}.webm`;
    ...
    try {
        ...
        if (isFinal && data.transcript) {
            // 📥 Download the transcript to the user's local machine
            const transcriptBlob = new Blob([data.transcript], { type: "text/plain" });
            const transcriptUrl = URL.createObjectURL(transcriptBlob);
            const aTxt = document.createElement("a");
            aTxt.style.display = "none";
            aTxt.href = transcriptUrl;
            aTxt.download = `Class_Transcript_${className}_${new Date().toLocaleDateString().replace(/\//g, "-")}.txt`;
